...
if( $map =~ "TRUE" ){	
	...
	foreach my $i (0 .. $#samplesInputs) {
		...
		#	my $cmd		= "bwa aln -n $ndiff $refGenome $path2currentfastq > $saidir/$samplesInputs[$i]\_1.sai";
			...
			my $path2currentfastq   = "$path2fastq/$samplesInputs[$i]\_1.fastq";			
...
if( $cleanbigwig =~ "TRUE" ){
	...
	foreach my $i (0 .. $#samples) {
		...
		my $cmd         = "mv $path2peakcalling/$sample\_fdr_$fdr/$sample.broadPeak $path2peakcalling/broadPeak/";
		`echo "$cmd" >> $QSUBint`;
		$cmd		= "mv $path2peakcalling/$sample\_fdr_$fdr/$sample.narrowPeak $path2peakcalling/narrowPeak/";
		`echo "$cmd" >> $QSUBint`;

		# Remove all lines starting with the character in @line2remove
		# Rename the original .wig file
		$cmd		= "cp $path2peakcalling/$sample\_fdr_$fdr/$sample.density.wig $path2peakcalling/$sample\_fdr_$fdr/$sample.density0.wig";
		`echo "$cmd"  >> $QSUBint`;
		foreach my $j (0 .. $#lines2remove) {
			# Remove lines from bigwig that do not correspond to specified chromosomes
			my $k = $j+1;
			my $cmd		= "sed '/$lines2remove[$j]/d' $path2peakcalling/$sample\_fdr_$fdr/$sample.density$j.wig > $path2peakcalling/$sample\_fdr_$fdr/$sample.density$k.wig";
			`echo "$cmd"  >> $QSUBint`;
			# Delete intermediary files
			$cmd		= "rm $path2peakcalling/$sample\_fdr_$fdr/$sample.density$j.wig";
			`echo "$cmd"  >> $QSUBint`;
		}
		...
		$cmd		= "wigToBigWig $path2peakcalling/$sample\_fdr_$fdr/$sample.density$finalcount.wig $chrlens $path2peakcalling/bigwig/$sample\.bw";
		`echo "$cmd"  >> $QSUBintWig`;
